...
function confirm_money(uid,money,type)
...
layer.open({
  ...
  area : ['480px' , '420px'],
  content: 'get_tixian.php?uid='+uid+'&money='+money+'&type='+type,
  });
}

function failed_money(uid,type)
...
layer.open({
  ...
  shadeClose: true, //点击遮罩关闭	层
  area : ['480px' , '300px'],
  content: 'get_tixian_failed.php?uid='+uid+'&type='+type,
  });
}
</script>
</head>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="dataTable">
  ...
  $sql = "SELECT *,sum(money) as money FROM `#@__tixian` where state=1 group by uid";
	...
	$r = $dosql->GetOne("SELECT name,account FROM $tbname WHERE id=$uid");
        ...
        $state = '<a style="cursor:pointer" onclick=" confirm_money('."'$uid'".','."'$money'".','."'$type'".');" >'.$state_success.'</a>';
        $state .= "&nbsp;&nbsp;&nbsp;";
        $state .= '<a style="cursor:pointer" onclick="return failed_money('."'$uid'".','."'$type'".');" >'.$state_faield.'</a>';
				break;
			case 1:
    ...
    $delstr = '<a href="tixian_save.php?action=del&id='.$row['id'].'" onclick="return ConfDel(0);">
