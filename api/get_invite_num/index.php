...
if(isset($token) && $token==$cfg_auth_key){
      ...
      #所有邀请推荐注册的会员
      $dosql->Execute("SELECT id FROM pmw_guide where recommender_openid='$openid'",$one);
      $nums_guide = $dosql->GetTotalRow($one);

      $dosql->Execute("SELECT id FROM pmw_agency where recommender_openid='$openid'",$two);
      ...
      #审核中
      if($type == 'shenhe'){
      $dosql->Execute("SELECT name,regtime FROM pmw_guide where recommender_openid='$openid' and checkinfo=0",$three);
      ...
      $dosql->Execute("SELECT name,regtime FROM pmw_agency where recommender_openid='$openid' and checkinfo=0",$four);
      ...
      #注册成功的会员
      $dosql->Execute("SELECT name,regtime FROM pmw_guide where recommender_openid='$openid' and checkinfo=1",$five);
      ...
      $dosql->Execute("SELECT name,regtime FROM pmw_agency where recommender_openid='$openid' and checkinfo=1",$six);
      ...
      #审核未通过的会员
      $dosql->Execute("SELECT name,regtime,account FROM pmw_un_guide where recommender_openid='$openid' and checkinfo=2",$seven);
      ...
      $dosql->Execute("SELECT name,regtime,account FROM pmw_un_agency where recommender_openid='$openid' and checkinfo=2",$eight);
